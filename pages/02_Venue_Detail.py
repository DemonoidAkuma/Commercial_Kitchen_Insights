...
# Get selected venue from Home page
...
allowed_venues = st.session_state.get(
    "venues"
)

if (
    allowed_venues != "ALL"
    and selected_venue not in allowed_venues
):
...
# Get selected venue from Home page
venue_name = st.session_state.get("selected_venue")
        ...
        with dashboard_right:
            ...
            if selected_metrics:
                ...
                for metric in selected_metrics:
                    ...
                    metric_key = available_metrics[metric]
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        name=metric,
                        text=[f"{x:.1f}" for x in trend_df[metric_key]],
...
def render_full_period_report(p, session):
    ...
    fc_vs_last = None
    waste_vs_last = None
    stock_movement = p.closing_stock - opening_stock
    stock_change = (
    ...
    with st.expander(f"📅 {p.period} ({p.report_type}-Day)", expanded=False):
        ...
        # -----------------------------
        purchases = p.purchases or p.cogs or 0
        closing_stock = p.closing_stock or 0
        ...
        if food_variance_percent < -1:
            ...
            # -----------------------------
            # 14-Day Report Disclaimer
            # -----------------------------
        if p.report_type == "14":
            commentary.append(
            "Please note: 14-day reports do not factor in stock adjustments, "
            "discount-backed COGS, or staff meal COGS. Results should be "
            "treated as indicative only."
        )
            commentary.append(
            "14-day report comparisons are benchmarked against the first "
            "14 days of the previous 28-day reporting period."
        )
        elif p.report_type == "28":
           commentary.append(
            "Please note: 28-day reports include stock adjustments to calculate "
            "true COGS; however, discount-backed COGS and staff meal COGS "
            "are not currently factored into results."
        )
       ...
       # =============================
        ...
        with st.expander("📎 Reference Documents", expanded=False):
            ...
            # Find FY folder containing this period
            period_path = None

            for fy in os.listdir(venue_path):
                ...
                if os.path.exists(candidate):
                    period_path = candidate
                    break

            if period_path and os.path.exists(period_path):
        ...
        if st.button(f"Generate Executive PDF - {p.period}"):
            ...
            if p.revenue:
                ...
                steakhouse_sales = getattr(
                    p,
                    "steakhouse_sales",
                    0
                )

                takeaway_sales = getattr(
                    p,
                    "takeaway_sales",
                    0
                )

                if steakhouse_sales > 0:

                    steakhouse_pct = (
                        steakhouse_sales / p.revenue
                    ) * 100

                    inhouse_pct = (
                        (p.revenue - steakhouse_sales)
                        / p.revenue
                    ) * 100
                ...
                if takeaway_sales > 0:
                    ...
                    takeaway_pct = (
                        takeaway_sales / p.revenue
                    ) * 100

                    inhouse_pct = (
                        (p.revenue - takeaway_sales)
                        / p.revenue
                    ) * 100
                 
            report_data = {

                "steakhouse_pct": steakhouse_pct,
                ...
                "food_cost": p.food_cost_percent,
                "food_target": target_percent,
                "previous_fc": (
                    prev_period.food_cost_percent
                    if prev_period else None
                ),

                "waste_pct": p.waste_percent,
                "previous_waste": (
                    prev_period.waste_percent
                    if prev_period else None
                ),
...
# --------------------------------
# Group: FY → Report Type → Period
# --------------------------------
report_tree = defaultdict(
    lambda: defaultdict(list)
)

for p in periods:
    fy = p.financial_year or "Unknown FY"
    report_tree[fy][p.report_type].append(p)

# --------------------------------
# Render Nested Reports
# --------------------------------
for fy in sorted(report_tree.keys(), reverse=True):
    ...
    with st.expander(
        ...
        for report_type in ["28", "14"]:

            if report_type not in report_tree[fy]:
                continue
            ...
            with st.expander(
                ...
                sorted_periods = sorted(
                    report_tree[fy][report_type],
                    key=lambda x: x.period,
                    reverse=True
                )

                for p in sorted_periods:
                    render_full_period_report(
                        p,
                        session
                    )